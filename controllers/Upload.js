...
const localHeight = 140;
const localWeight = 40;
...
exports.doCalc = async (req, res) => {
  ...
  if (typeof req.body.weight == "undefined" || req.body.weight.trim == "") {
    if (req.body.height < 140) {
      return res
        .status(500)
        .json({ Messages: "Height is too low", status: 0 })
    }
    else {
      let enterHeight = Number(req.body.height);
      let difference = enterHeight - localHeight;
      weight = localWeight + difference;
    }
  }
